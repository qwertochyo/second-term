document.addEventListener("DOMContentLoaded", function () {
  ...
  const firstSelect = document.getElementById("firstSort");
  const secondSelect = document.getElementById("secondSort");

  setSortSelects(recipes, sortForm);

  filterButton.addEventListener("click", () => {
    filterTable(recipes, dataForm, idTable);
    resetSort(buildings, sortForm, idTable);
  });
  resetFilterButton.addEventListener("click", () => {
    clearFilter(recipes, dataForm, idTable);
    resetSort(buildings, sortForm, idTable);
  });
  sortButton.addEventListener("click", () => sortTable(sortForm, idTable));
  resetSortButton.addEventListener("click", () => {
    resetSort(recipes, sortForm, idTable);
    clearFilter(buildings, dataForm, idTable);
  });

  firstSelect.addEventListener("change", () => {
    changeNextSelect(firstSelect, "secondSort");
    changeNextSelect(secondSelect, "thirdSort");
  });
  secondSelect.addEventListener("change", () => changeNextSelect(secondSelect, "thirdSort"));
